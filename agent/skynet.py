...
class Skynet:
    ...
    def run_turn(self, audio_bytes: bytes, session: dict) -> dict:
        ...
        transcript_result = self.transcriber.transcribe(audio_bytes)
        ...
        audio_bytes = self.tts.synthesizer.speak(response_text)
        if followup_text:
            audio_bytes += self.tts.synthesizer.speak(followup_text)
    ...
    def _generate_response(self, intent: str, transcript: str, entities: dict, session: dict) -> dict:
        ...
        try:
            ...
            fallback = self._fallback_response(intent, entities)
            return {
                "response": fallback,
                "followup": None,
            }
            ...
            return {
                "response": "Sorry bhai, technical issue. Ek second mein dobara try karo.",
                "followup": None,
            }
